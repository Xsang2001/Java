...
void DoiHSL(string manv,float hsl){
	...
	for(DSNV * p=first;p!=NULL;p=p->next){
		if(p->nv.manv == manv){
			p->nv.hsl=hsl;
			cout<<"Thong tin nhan vien la : "<<p->nv.manv<<";"<<p->nv.hoten<<";"<<p->nv.loaihopdong<<";"<<p->nv.hsl<<endl;
			return;
		}
 ...
 void luufileBin(){
 	...
 	string loaihopdong;
 	fflush(stdin);
 	cout<<"\n Nhap vao 1 loai hop dong :\n";
 	getline(cin,loaihopdong);
 	for(DSNV *p=first;p!=NULL;p=p->next)
 		if(p->nv.loaihopdong == loaihopdong)
